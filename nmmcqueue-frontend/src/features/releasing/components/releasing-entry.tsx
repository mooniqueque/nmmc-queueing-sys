...
import { Play, Printer, WarningCircle, X } from "@phosphor-icons/react";
import { useEffect, useMemo, useState, useTransition } from "react";
import { assignTicket, callNextWindow, callTicket, noShowTicket, updatePatientDemographics } from "../actions";
                            ...
                            {!hasActivePatient ? (
                                <div className="flex flex-col items-center justify-center text-center rounded-2xl border border-dashed border-slate-200 bg-white p-12 lg:p-20 mt-4 shadow-sm">
                                    ...
                                    <Button
                                        ...
                                        disabled={isPending}
                                        className="mt-8 h-14 px-10 text-lg font-semibold bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 rounded-xl shadow-sm"
                                    >
                                    ...
                                    <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-6">
                                        ...
                                        <div className="flex flex-wrap gap-3 shrink-0">
                                            <Button
                                                ...
                                                variant="outline"
                                                className="h-11 px-6 text-base font-semibold text-emerald-700 border border-emerald-200 bg-emerald-50 hover:bg-emerald-100 transition-all shadow-sm rounded-xl"
                                            >
                                                Call Again
                                            </Button>
                                            <Button
                                                ...
                                                disabled={isPending}
                                                className="h-11 px-6 text-base font-semibold transition-all shadow-sm rounded-xl border-rose-200 text-rose-600 bg-rose-50/50 hover:bg-rose-50 hover:text-rose-700"
                                            >
                                            ...
                                            <Button
                                                ...
                                                className={`h-11 px-8 text-base font-semibold transition-all shadow-sm rounded-xl border ${!currentVisit?.departmentId ? 'opacity-50 cursor-not-allowed text-emerald-700 bg-emerald-50 border-emerald-200' : 'bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border-emerald-200'}`}
                                            >
                                    ...
                                    <Card className="rounded-2xl border border-slate-100 shadow-sm">
                                        <CardContent className="p-6 space-y-6">
                                            <div>
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                                        <div className="text-sm font-semibold text-slate-600 mb-1">Disposition</div>
                                                        <div className="text-xl font-semibold text-slate-900">{currentVisit?.disposition || "None"}</div>
                                                    </div>
                                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                                        <div className="text-sm font-semibold text-slate-600 mb-1">Classification</div>
                                                        <div className="text-xl font-semibold text-slate-900">{activeClassificationLabel}</div>
                                                    </div>
                                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 ring-1 ring-emerald-100/70">
                                                        <div className="text-sm font-semibold text-slate-600 mb-1">Clinic Department</div>
                                                        <div className="text-xl font-semibold text-slate-900">{triageAssignedDepartmentName}</div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                                    <span className="text-sm font-semibold text-muted-foreground mb-1 block">Blood Pressure</span>
                                                    <div className="flex items-baseline gap-1">
                                                        <span className="text-2xl font-semibold tracking-tight text-foreground">{currentVisit?.bloodPressure || "--/--"}</span>
                                                        <span className="text-sm text-muted-foreground">mmHg</span>
                                                    </div>
                                                </div>
                                                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                                    <span className="text-sm font-semibold text-muted-foreground mb-1 block">Temperature</span>
                                                    <div className="flex items-baseline gap-1">
                                                        <span className="text-2xl font-semibold tracking-tight text-foreground">{currentVisit?.temperature || "--"}</span>
                                                        <span className="text-sm text-muted-foreground">°C</span>
                                                    </div>
                                                </div>
                                                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 sm:col-span-2 lg:col-span-1">
                                                    <span className="text-sm font-semibold text-muted-foreground mb-1 block">Chief Complaint</span>
                                                    <span className="text-base font-medium text-foreground line-clamp-2">{currentVisit?.chiefComplaint || "None recorded"}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                                <Label className="text-base font-semibold text-gray-700">Contact Number</Label>
                                                <Input
                                        ...
                                        <div className="mt-6 space-y-2">
                                            <Label className="text-base font-semibold text-gray-700">Full Address</Label>
                                            <Input
