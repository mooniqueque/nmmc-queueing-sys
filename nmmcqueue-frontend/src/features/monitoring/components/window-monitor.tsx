...
export default function WindowMonitor() {
    ...
    useEffect(() => {
        getDepartmentsVideos().then(res => {
            if (res.success) {
                // Look for the special department loop
                const dept = res.data.find((d: any) => d.name === 'CASHIER / REGISTRATION');
                if (dept) {
    ...
    return (
        <div className="w-full h-screen bg-slate-50 flex flex-col font-sans text-slate-800 overflow-hidden">
            {/* ... Rest of the component remains similar, but using videoUrl ... */}
            {/* HEADER */}
