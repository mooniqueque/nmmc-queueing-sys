...
export default function DepartmentMonitor({ departmentId }: DepartmentMonitorProps) {
    const currentTime = useCurrentTime();
    const { windows, loading } = useWindowMonitor(departmentId);
    ...
    useEffect(() => {
        getDepartments().then(res => {
            if (res.success) {
                ...
                if (dept) setDepartmentName(dept.name.toUpperCase());
            }
        });
    }, [departmentId]);
