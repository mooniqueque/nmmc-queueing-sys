...
export async function createQueueOption(departmentName: string, option: string) {
    ...
    if (!normalized) return { success: false, error: "Queue option cannot be empty." };

    const department = await db.department.findUnique({
        where: { name: departmentName.trim() },
        select: { id: true, queueOptions: { select: { option: true } } }
    });
    ...
    if (effectiveOptions.includes(normalized)) return { success: false, error: "Queue option already exists." };
...
export async function deleteQueueOption(departmentName: string, option: string) {
    ...
    if (next.length === effectiveOptions.length) return { success: false, error: "Queue option not found." };
